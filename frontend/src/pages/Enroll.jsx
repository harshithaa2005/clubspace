...
const Enroll = () => {
  ...
  const targetEvent = queryParams.get('event') || 'Event 2026';

  const [formData, setFormData] = useState({ event: targetEvent, name: '', email: '', phone: '', department: '', comments: '' });
      ...
      <div className="animate-fade-up" style={{ maxWidth: '450px', margin: '0 auto', background: 'rgba(255,255,255,0.03)', padding: '40px', borderRadius: '15px', border: '1px solid rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ textAlign: 'center' }}>
          <div style={{ marginBottom: '20px', textAlign: 'left' }}>
            <label style={{ fontSize: '14px', color: '#ccc', marginLeft: '5px' }}>Enrolling For:</label>
            <input className="form-control" name="event" value={formData.event} readOnly style={{ width: '100%', padding: '10px', background: 'rgba(255,60,87,0.1)', color: '#ff3c57', fontWeight: 'bold', cursor: 'not-allowed', marginTop: '5px' }} />
          </div>

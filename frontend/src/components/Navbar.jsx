  ...
  return (
    <header style={{ width: '100%' }}>
      <nav className="navbar">
        <div className="logo">
          <Link style={{ color: "var(--logo-color, #fff)", textDecoration: "none" }} to={user?.role === 'admin' ? '/admin' : '/'} onClick={() => setIsOpen(false)}>ClubSpace</Link>
        </div>
        
        <button className="hamburger sm:block md:hidden focus:outline-none" onClick={toggleMenu} style={{ background: 'none', border: 'none', color: 'white', fontSize: '28px', cursor: 'pointer' }}>
          <i className={isOpen ? "fas fa-times" : "fas fa-bars"}></i>
        </button>

        <ul className={`nav-links ${isOpen ? 'active' : ''}`} onClick={() => setIsOpen(false)}>
          {user?.role === 'admin' ? (
            <>
